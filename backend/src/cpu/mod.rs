...
impl<T: SerialInterface> CPU<T> {
    ...
    pub fn step(&mut self, bus: &mut T) -> Result<u8, String> {
        ...
        info!("{:04X}  {:<9} {:<32} A:{:02X} X:{:02X} Y:{:02X} P:{:02X} SP:{:02X} CYC:{}",
            starting_program_counter,
            opcode_bytes,
            operand_display,
        ...
        let cycles = (next_instruction.operation)(&next_instruction, &mut self.registers, bus)?;
        self.total_cycles += cycles as u32;

        if is_page_crossed && next_instruction.instruction_type.has_page_crossing_penalty() {
            self.total_cycles += 1;
        }
    ...
    fn fetch_next_instruction(&mut self, bus: &mut T) -> Result<(Instruction<T>, bool), String> {
        ...
        let (operand, is_page_crossed) = self.fetch_operand(instruction.addressing_mode, bus);
        instruction.operand = operand;

        Ok((instruction, is_page_crossed))
    }

    fn fetch_operand(&mut self, addressing_mode: AddressingMode, bus: &mut T) -> (Option<Operand>, bool) {
        match addressing_mode {
            AddressingMode::ZeroPageIndexedX => {
                ...
                (Some(Operand::Address(effective_address as u16)), false)
            }
            AddressingMode::ZeroPageIndexedY => {
                let address = bus.read_byte(self.registers.program_counter);
                self.registers.increment_program_counter(1);
            ...
            AddressingMode::AbsoluteIndexedX => {
                ...
                (Some(Operand::Address(effective_address)), self.page_crossed(address, effective_address))
            }
            AddressingMode::AbsoluteIndexedY => {
                let address = bus.read_word(self.registers.program_counter);
                self.registers.increment_program_counter(2);
            ...
            AddressingMode::IndirectIndexedX => {
                ...
                (Some(Operand::Address(effective_address)), false)
            }
            AddressingMode::IndirectIndexedY => {
                let address = bus.read_byte(self.registers.program_counter);
                self.registers.increment_program_counter(1);
                ...
                (Some(Operand::Address(effective_address)), self.page_crossed(base_address, effective_address))
            }
            AddressingMode::Implicit => (None, false),
            AddressingMode::Accumulator => (Some(Operand::Value(self.registers.accumulator)), false),
            AddressingMode::Immediate => {
                let value = bus.read_byte(self.registers.program_counter);
                self.registers.increment_program_counter(1);

                (Some(Operand::Value(value)), false)
            }
            AddressingMode::ZeroPage => {
                let address = bus.read_byte(self.registers.program_counter);
                self.registers.increment_program_counter(1);

                (Some(Operand::Address(address as u16)), false)
            }
            AddressingMode::Absolute => {
                let address = bus.read_word(self.registers.program_counter);
                self.registers.increment_program_counter(2);

                (Some(Operand::Address(address)), false)
            }
            AddressingMode::Relative => {
            ...
            AddressingMode::Indirect => {
                ...
                (Some(Operand::Address(effective_target)), false)
            }
        }
    }

    fn page_crossed(&self, base_address: u16, final_address: u16) -> bool {
        (base_address >> 8) != (final_address >> 8)
    }
}
...
mod tests {
    ...
    fn fetch_next_instruction_reads_at_current_pc() {
        ...
        assert_eq!(instruction.unwrap().0.opcode, 0xA9);
    }
    ...
    fn implied_addressing_return_no_operand() {
        ...
        let (operand, _) = cpu.fetch_operand(AddressingMode::Implicit, &mut test_bus);
    ...
    fn immediate_addressing_returns_correct_operand() {
        ...
        let (operand, _) = cpu.fetch_operand(AddressingMode::Immediate, &mut test_bus);
    ...
    fn absolute_addressing_returns_correct_operand() {
        ...
        let (operand, _) = cpu.fetch_operand(AddressingMode::Absolute, &mut test_bus);
    ...
    fn zero_page_addressing_returns_correct_operand() {
        ...
        let (operand, _) = cpu.fetch_operand(AddressingMode::ZeroPage, &mut test_bus);
    ...
    fn absolute_x_indexed_addressing_returns_correct_operand() {
        ...
        let (operand, _) = cpu.fetch_operand(AddressingMode::AbsoluteIndexedX, &mut test_bus);
    ...
    fn absolute_y_indexed_addressing_returns_correct_operand() {
        ...
        let (operand, _) = cpu.fetch_operand(AddressingMode::AbsoluteIndexedY, &mut test_bus);
        assert_eq!(operand, Some(Operand::Address(0x3132)));
    }
    ...
    fn zero_paged_x_indexed_addressing_returns_correct_operand() {
        ...
        let (operand, _) = cpu.fetch_operand(AddressingMode::ZeroPageIndexedX, &mut test_bus);
    ...
    fn zero_paged_y_indexed_addressing_returns_correct_operand() {
        ...
        let (operand, _) = cpu.fetch_operand(AddressingMode::ZeroPageIndexedY, &mut test_bus);
        assert_eq!(operand, Some(Operand::Address(0x82)));
    }
    ...
    fn indirect_addressing_returns_correct_operand() {
        ...
        let (operand, _) = cpu.fetch_operand(AddressingMode::Indirect, &mut test_bus);
    ...
    fn preindexed_indirect_x_addressing_returns_correct_operand() {
        ...
        let (operand, _) = cpu.fetch_operand(AddressingMode::IndirectIndexedX, &mut test_bus);
    ...
    fn preindexed_indirect_y_addressing_returns_correct_operand() {
        ...
        let (operand, _) = cpu.fetch_operand(AddressingMode::IndirectIndexedY, &mut test_bus);
    ...
    fn relative_addressing_returns_correct_operand() {
        ...
        let (operand, _) = cpu.fetch_operand(AddressingMode::Relative, &mut test_bus);
